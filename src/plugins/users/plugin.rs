use axum::{Router, routing::{post, get, put, delete}, Json, extract::Path, extract::Extension};
use sqlx::PgPool;
use crate::kernel::Plugin;
use crate::plugins::users::models::{CreateUser, UpdateUser};
use crate::plugins::users::handlers::{create_user, list_users, get_user, update_user, delete_user, grant_admin, create_admin};
...
impl Plugin for UsersPlugin {
    ...
    let p_admin = self.pool.clone();
        ...
        Router::new()
            ...
            .route("/:id", get(move |ext: Extension<Option<DynCache>>, Path(id): Path<uuid::Uuid>| {
                let pool = p_get.clone();
                async move { get_user(ext, pool, Path(id)).await }
            }))
            .route("/:id", put(move |ext: Extension<Option<DynCache>>, Path(id): Path<uuid::Uuid>, Json(payload): Json<UpdateUser>| {
                let pool = p_update.clone();
                async move { update_user(ext, pool, Path(id), Json(payload)).await }
            }))
            .route("/:id", delete(move |ext: Extension<Option<DynCache>>, Path(id): Path<uuid::Uuid>| {
                let pool = p_delete.clone();
                async move { delete_user(ext, pool, Path(id)).await }
            }))
            .route("/:id/grant_admin", post(move |ext: Extension<Option<DynCache>>, Path(id): Path<uuid::Uuid>, auth: crate::plugins::auth::handlers::AuthUser| {
                ...
                async move { grant_admin(ext, pool, auth, Path(id)).await }
                    ...
                    .route("/admin", post(move |ext: Extension<Option<DynCache>>, auth: crate::plugins::auth::handlers::AuthUser, Json(payload): Json<CreateUser>| {
