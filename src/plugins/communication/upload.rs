    ...
    while let Some(field) = multipart.next_field().await.map_err(|e| AppError::from((StatusCode::BAD_REQUEST, format!("multipart error: {}", e))))? {
        if let Some(filename_orig) = field.file_name() {
            ...
            if data.len() > MAX_SIZE {
                return Err(AppError::from((StatusCode::BAD_REQUEST, format!("file too large (max {} MB)", MAX_SIZE / 1024 / 1024))));
            }
            ...
            if !allowed_mimes.contains(&mime) {
                return Err(AppError::from((StatusCode::BAD_REQUEST, format!("unsupported file type: {}", mime))));
            }
            // extension from mime
            let ext = match mime {
                "image/png" => "png",
                ...
                _ => {
                    ...
                    std::path::Path::new(&filename).extension().and_then(|s| s.to_str()).unwrap_or("bin")
                }
            };
                ...
                if let Ok(reader) = ImageReader::new(std::io::Cursor::new(&data)).with_guessed_format() {
                    if let Ok(img) = reader.decode() {
                        ...
                        if w > MAX_DIM || h > MAX_DIM {
                            ...
                            if resized.write_to(&mut std::io::Cursor::new(&mut out), fmt).is_ok() {
                                final_bytes = out;
                            }
                        }
                    }
            ...
            path.push(&fname);
            tokio::fs::write(&path, &final_bytes).await.map_err(|e| AppError::from((StatusCode::INTERNAL_SERVER_ERROR, format!("write error: {}", e))))?;
