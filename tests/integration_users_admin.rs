...
async fn integration_create_admin_flow() -> anyhow::Result<()> {
    ...
    let _ = ecoblock_api_kernel::plugins::users::repo::insert_user_with_admin(&pool, "itadmin", "itadmin@example.com", "password123", true).await.map_err(|e| anyhow::anyhow!("repo error: {:?}", e))?;
    let login = client.post(&format!("{}/auth/login", base)).json(&serde_json::json!({"username":"itadmin","password":"password123"})).send().await?;
    ...
    let is_admin = ecoblock_api_kernel::plugins::users::repo::is_admin(&pool, created_uuid).await.map_err(|e| anyhow::anyhow!("repo error: {:?}", e))?;
...
async fn integration_create_admin_forbidden_for_non_admin() -> anyhow::Result<()> {
    ...
    let (base, server_handle, _jwt_secret, pool, _guard) = setup_and_spawn(&test_db).await?;
    let client = reqwest::Client::new();
    let _ = ecoblock_api_kernel::plugins::users::repo::insert_user(&pool, "regular", "regular@example.com", "password123").await.map_err(|e| anyhow::anyhow!("repo error: {:?}", e))?;
    let login = client.post(&format!("{}/auth/login", base)).json(&serde_json::json!({"username":"regular","password":"password123"})).send().await?;
    assert_eq!(login.status(), StatusCode::OK);
    let token_body: Value = login.json().await?;
    let token = token_body["token"].as_str().unwrap();
    ...
    assert_eq!(body["error"].as_str().unwrap(), "onlyAdminCanCreateAdminAccounts");
    server_handle.abort();
    let _ = server_handle.await;
    Ok(())
}
...
async fn integration_create_admin_validation_errors() -> anyhow::Result<()> {
    ...
    let (base, server_handle, _jwt_secret, pool, _guard) = setup_and_spawn(&test_db).await?;
    let client = reqwest::Client::new();
    let _ = ecoblock_api_kernel::plugins::users::repo::insert_user_with_admin(&pool, "itadmin2", "itadmin2@example.com", "password123", true).await.map_err(|e| anyhow::anyhow!("repo error: {:?}", e))?;
    let login = client.post(&format!("{}/auth/login", base)).json(&serde_json::json!({"username":"itadmin2","password":"password123"})).send().await?;
    assert_eq!(login.status(), StatusCode::OK);
    let token_body: Value = login.json().await?;
    let token = token_body["token"].as_str().unwrap();
    let create_bad_email = client.post(&format!("{}/users/admin", base)).bearer_auth(token).json(&serde_json::json!({"username":"bad","email":"bad-email","password":"password123"})).send().await?;
    ...
    let create_short_pwd = client.post(&format!("{}/users/admin", base)).bearer_auth(token).json(&serde_json::json!({"username":"bad2","email":"bad2@example.com","password":"short"})).send().await?;
