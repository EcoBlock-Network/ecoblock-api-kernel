...
async fn communication_blog_crud_and_list() -> anyhow::Result<()> {
    ...
    let blog_plugin =
        ecoblock_api_kernel::plugins::communication::blog::plugin::BlogPlugin::new(pool.clone());
    ...
    // get
    let one = client
        .get(&format!("{}/communication/blog/{}", base, id))
        .send()
        .await?;
    ...
    // update
    let upd = client
        .put(&format!("{}/communication/blog/{}", base, id))
        .json(&serde_json::json!({"title":"Hello2"}))
        .send()
        .await?;
    ...
    // delete
    let del = client
        .delete(&format!("{}/communication/blog/{}", base, id))
        .send()
        .await?;
